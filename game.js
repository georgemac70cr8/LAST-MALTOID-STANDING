...
async function load(){const rows=await sb("game_state?id=eq.1&select=state");if(rows&&rows[0])return rows[0].state;const data=seed();await sb("game_state",{method:"POST",body:JSON.stringify({id:1,state:data})});return data;}
async function save(state){await sb("game_state?id=eq.1",{method:"PATCH",body:JSON.stringify({state})});}
